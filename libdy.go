...
func DeleteItemSingle(svc *dynamodb.DynamoDB, table, key, val string) error {
	start := time.Now()
	input := &dynamodb.DeleteItemInput{
		Key:       map[string]*dynamodb.AttributeValue{key: {S: aws.String(val)}},
		TableName: aws.String(table),
	}
	...
	err = backoff.Retry(op, backoff.NewExponentialBackOff())
	if err != nil {
		return fmt.Errorf("DeleteItemSingle failed after %v: %w", time.Since(start), err)
	}

	if rerr != nil {
		return fmt.Errorf("DeleteItemSingle failed: %w", rerr)
	}
